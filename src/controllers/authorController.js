const { titleCheck } = require('../middlewares/validator')
...
const createAuthor = async (req, res) => {
    try {
        ...
        if (!validator.isEmail(email)) res.status(400).send({ status: false, message: `${email} is not valid` })
            ...
            const aut = await authorModel.findOne({ email: email })
            if(aut) res.status(400).send({ status: false, message: `${email} is already registered with this account` })
...
const authorLogin = async (req, res) => {
    try {
        ...
        if (!author|| author == null) return res.status(401).send({ status: false, message: 'Invalid email or password' })
        else {
